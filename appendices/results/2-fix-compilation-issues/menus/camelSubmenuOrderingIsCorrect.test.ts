...
describe('menus - camelSubmenuOrderingIsCorrect', () => {
    ...
    it('Validates the order of submenu items based on group indices (1 to 9).', async function() {
        ...
        createdFileName = `camel-ordering-${Date.now()}.yaml`;
        ...
        const camelFileSubmenu = await (camelMenuItem as any).getSubmenu();
        ...
        const camelFolderSubmenu = await (camelMenuItemFolder as any).getSubmenu();
        await driver.sleep(300);
