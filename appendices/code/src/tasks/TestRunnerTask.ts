...
export abstract class TestRunner extends Task {
	...
	public async execute(): Promise<void> {
		const log = this.logger.withScope('TestRunner');
		log.info(`Starting test execution: ${this.label}`);
		try {
			await tasks.executeTask(this);
			await this.waitForEnd();
			log.info(`Test execution completed: ${this.label}`);
		} catch (error) {
			log.error(`Test execution failed: ${this.label} - ${error}`);
		} finally {
			log.debug('Refreshing all trees after test run');
		...
		await new Promise<void>((resolve) => {
			const disposable = tasks.onDidEndTask((e) => {
				if (e.execution.task.name === this.label) {
					log.debug(`Task finished: ${this.label}`);
					disposable.dispose();
					resolve();
				}
			});
